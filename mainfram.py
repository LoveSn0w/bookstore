...
#        temparray,size=self.warfobj(result)
    ...
    def initordersearch(self):
        ...
        orderSQL.connectdb()
        (result,description,count,col)=orderSQL.searchtableinfo_byparams(['orders','users'],['oid','ouser','orders.state','payment','ordertime','name'],['ouser'],['uid'])
        self .drawtable(result,description,count,col,self.orderdatacontent)
        orderSQL.closedb()
    def initbuysearch(self):
        buySQL=PostGreSQLTool.DBmanager()
        buySQL.connectdb()
        (result,description,count,col)=buySQL.searchtableinfo_byparams(['orders','orderbook','book','users'],['bookid','title','count(bookid) as buy_time'],['bookid','orderid','ouser','orders.state>'],['book.bid','oid','uid','3 group by bookid,title'])
